...
export type CollaborationContextTab = "conversation" | "email" | "settings";
...
export function CollaborationContextTabs({ selected, onSelect }: {
  ...
  const tabs = ["conversation", "email", "settings"] as const;
  ...
  return <div role="tablist" aria-label="Collaboration context" className="flex shrink-0 gap-5 border-b border-[var(--workspace-border)]">
    ...
    >{tab === "conversation" ? "Conversation" : tab === "email" ? "Email" : "Settings"}</button>)}
  </div>;
}

export function CollaborationLifecycleSettings({ resolved, pending, themeMode, onTransition }: {
  resolved: boolean;
  pending: boolean;
  ...
  return <section aria-labelledby="collaboration-lifecycle-heading" className="mt-6 space-y-3 border-t border-[var(--workspace-border)] pt-5">
    <h3 id="collaboration-lifecycle-heading" className="text-sm font-medium text-[var(--workspace-text)]">Collaboration</h3>
    <p className="text-sm text-[var(--workspace-text-muted)]">Status: <span data-collaboration-lifecycle-status>{resolved ? "Resolved" : "Active"}</span></p>
    <button ref={triggerRef} type="button" disabled={pending}
      onClick={() => { if (resolved) onTransition(); else setConfirmOpen(true); }}
      className="min-h-11 rounded-full border border-[var(--workspace-border)] bg-[var(--workspace-card)] px-4 text-sm text-[var(--workspace-text)] hover:bg-[var(--workspace-hover-surface)] disabled:text-[var(--workspace-text-muted)]">
      {pending ? "Updating…" : resolved ? "Reopen collaboration" : "Resolve collaboration"}
    </button>
