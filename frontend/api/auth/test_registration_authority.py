"""Offline tests for invite-bound Auth0 registration authority."""
...
class RegistrationAuthorityTests(unittest.TestCase):
    ...
    def test_grant_store_is_opaque_nx_and_time_bounded(self):
        ...
        self.assertIsNone(store.get(GRANT, now=NOW + 600))
    ...
    def test_issue_team_invite_grant_binds_email_client_state_and_invite(self):
        commands = MemoryCommands()
        store = registration_authority.RegistrationGrantStore(commands)
        ...
        self.assertEqual(record.oauth_state, STATE)
    ...
    def test_validated_team_invite_redirect_gets_server_backed_acr_grant(self):
        commands = MemoryCommands()
        store = registration_authority.RegistrationGrantStore(commands)
        location = (
        ...
        redirect = [value for name, value in response.headers if name.lower() == "location"][0]
        ...
        self.assertTrue(any(name.lower() == "set-cookie" for name, _ in response.headers))
    ...
    def test_action_validation_accepts_only_exact_server_backed_binding(self):
        commands = MemoryCommands()
        store = registration_authority.RegistrationGrantStore(commands)
        ...
        self.assertEqual(json.loads(valid.body), {"authorized": True})
        ...
        for changes in (
            ...
            with self.subTest(changes=changes):
                ...
                self.assertEqual(json.loads(denied.body), {"authorized": False})
    ...
    def test_action_validation_is_fail_closed_for_auth_context_and_configuration(self):
        ...
        cases = (
            ...
            ("POST", self._headers(secret="wrong-secret-" + "x" * 32), ENVIRONMENT, 403),
            ("POST", (("host", "evil.example"), *self._headers()[1:]), ENVIRONMENT, 403),
            ("POST", tuple(pair for pair in self._headers() if pair[0] != "content-type"), ENVIRONMENT, 415),
            ("POST", self._headers(), {key: value for key, value in ENVIRONMENT.items() if key != "CUEVION_AUTH0_REGISTRATION_AUTHORITY_SECRET"}, 503),
        )
