...
All Auth-A account, evidence, operation, receipt, session, and security-event timestamps are exact built-in Python `int` values: integral Unix UTC seconds in the inclusive range `0..253402300799`. `bool`, integer subclasses, floats, negative values, milliseconds, microseconds, implicit defaults, and timezone-naive interpretations are invalid. Existing lifecycle ordering remains mandatory. The inactive PostgreSQL foundation maps this contract only to exact UTC-aware whole-second timestamps; no floating-point timestamp round trip is authority evidence.
...
Auth-A itself selects or creates no storage implementation. The separate inactive `cuevion_db` PostgreSQL schema-one foundation now concretizes only the seven-table account authority shape and offline migration SQL. It supplies no repository, Engine, connection, credentials, session store, login behavior, route, or activation. Its remaining activation requirements are documented in `cuevion_db/DATABASE_FOUNDATION_ACTIVATION_REQUIREMENTS.md`.
