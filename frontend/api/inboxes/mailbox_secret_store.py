...
LegacyMailboxSecretCleanupStatus = Literal[
    "deleted",
    "already_absent",
    "invalid",
    "v2_missing",
...
class LegacyMailboxSecretCleanupResult(TypedDict):
    status: LegacyMailboxSecretCleanupStatus
    error: dict | None
...
def _delete_durable_record_with_outcome(
    config: dict,
    store_key: str,
) -> tuple[Literal["deleted", "already_absent"] | None, dict | None]:
    payload, error = _perform_rest_request(
        config,
        "POST",
        f"/del/{quote(store_key, safe='')}",
    ...
    if not isinstance(payload, dict) or type(payload.get("result")) is not int:
        return None, _build_error(
            "Mailbox secret store returned an unreadable deletion response."
        )

    deleted_count = payload["result"]
    if deleted_count == 1:
        return "deleted", None
    if deleted_count == 0:
...
def _decrypt_secret_record_for_cleanup(
    encryption_key: bytes,
    owner_email: str,
    mailbox_id: str,
    encrypted_record: dict,
) -> tuple[dict | None, Literal["malformed", "decryption_failed"] | None]:
    try:
        nonce = _decode_base64url(encrypted_record["nonce"])
        ciphertext = _decode_base64url(encrypted_record["ciphertext"])
    except (binascii.Error, ValueError, TypeError):
        return None, "malformed"
    if len(nonce) != MAILBOX_SECRET_NONCE_BYTES or len(ciphertext) < 16:
        return None, "malformed"
    ...
    return {
        "v": MAILBOX_SECRET_ENCRYPTED_SCHEMA_VERSION,
        "mailboxId": _normalize_mailbox_id(mailbox_id),
        "updatedAt": encrypted_record["updatedAt"],
        "imapPassword": imap_password,
        "smtpPassword": smtp_password,
    }, None
...
def _cleanup_failure(
    ...
    code: str,
    message: str,
) -> LegacyMailboxSecretCleanupResult:
    return {
        "status": status,
        "error": {
            "code": code,
...
def cleanup_legacy_mailbox_secret_v1(
    owner_email: str,
    mailbox_id: str,
    use_same_credentials: bool,
) -> LegacyMailboxSecretCleanupResult:
    """Delete one V1 record only after an independent usable V2 decrypt."""
    normalized_owner_email = normalize_auth_email(owner_email)
    normalized_mailbox_id = _normalize_mailbox_id(mailbox_id)
    if (
        not normalized_owner_email
        or not _is_storable_mailbox_id(normalized_mailbox_id)
        or type(use_same_credentials) is not bool
    ):
        return _cleanup_failure(
            "invalid",
            "mailbox_secret_cleanup_invalid",
            "Mailbox secret cleanup input is invalid.",
        )
    ...
    encrypted_record, encrypted_error = _read_durable_record(
        config,
        build_encrypted_mailbox_secret_key(
            normalized_owner_email,
            normalized_mailbox_id,
        ),
    )
    if encrypted_error:
        if encrypted_error.get("code") == "mailbox_secret_malformed":
            return _cleanup_failure(
                "v2_malformed",
    ...
    if not isinstance(encrypted_record, dict) or _validate_encrypted_record_shape(
        encrypted_record
    ):
        return _cleanup_failure(
            "v2_malformed",
    ...
    decrypted_record, decrypt_error = _decrypt_secret_record_for_cleanup(
        encryption_key,
        normalized_owner_email,
        normalized_mailbox_id,
        encrypted_record,
    )
    if decrypt_error == "decryption_failed":
        return _cleanup_failure(
            "v2_decryption_failed",
    ...
    imap_password = decrypted_record.get("imapPassword")
    smtp_password = decrypted_record.get("smtpPassword")
    ...
    if not use_same_credentials and (
        not isinstance(smtp_password, str) or not smtp_password
    ):
        return _cleanup_failure(
            "v2_unusable",
            "mailbox_secret_v2_unusable",
    ...
    delete_outcome, delete_error = _delete_durable_record_with_outcome(
        config,
        build_mailbox_secret_key(
            normalized_owner_email,
            normalized_mailbox_id,
        ),
    )
    if delete_error or delete_outcome not in {"deleted", "already_absent"}:
        return _cleanup_failure(
            "delete_failed",
    ...
    return {"status": delete_outcome, "error": None}
