...
import { composeCollaborationPrioritySource } from '../../src/lib/collaborationPriority';
import { resolvePrioritySource } from '../../src/lib/prioritySource';
import { useCollaborationSummaries } from '../../src/lib/useCollaborationSummaries';
import { createRoot } from 'react-dom/client';
import { lookupCollaborationForOwner, readCollaborationForOwner } from '../../src/lib/collaborationOwnerReadApi';
import { createCollaborationSummaryStore, lookupCollaborationSummary, lookupActiveCollaborationSummary, isCurrentCollaborationOpenBinding } from '../../src/lib/collaborationSummaryStore';
import { deriveCollaborationOwnerSourceLocator } from '../../src/lib/collaborationOwnerSourceLocator';
...
import { CollaborationContextTabs, CollaborationLifecycleSettings } from '../../src/components/collaboration/CollaborationContextControls';
import { isCollaborationOwnerReadFailureRetryable, resolveMessageBodyRenderMode, EmailHtmlStage, renderPlainMessageParagraph, getQuotedParagraphStartIndex, parseQuotedSections, hasReliableQuotedContent, shouldShowInAttachmentList, shouldRenderKnownProviderHtmlOnLightCanvas, isComposeGeneratedHtml, resolveDesktopThreadTimestamp, normalizeSenderLearningKey, SignatureBlock } from 'cuevion-fixture-reader';
/* COPY */
/* TIME */
...
const sourceMail = {
  id: 'mail', providerMessageId: 'source-123', serverMailboxId: 'main', sender: 'Alex Morgan', from: new URLSearchParams(location.search).has('formatted-sender') ? 'Alex Morgan <alex@example.test>' : 'alex@example.test', to: 'Rutger <rutger@example.test>, Jamie <jamie@example.test>', cc: 'Dan <dan@example.test>', subject: canonical.source.subject,
...
const summary=()=>({collaborationId:canonical.collaborationId,workspaceId,mailboxId:'main',sourceRef:{provider:'google',providerMessageId:'source-123'},state:canonical.state,updatedAt:canonical.updatedAt,viewerAccess:'owner'});
// Canonical transport persistence is fixture-only; the real hook rehydrates via list_summaries.
const persisted=sessionStorage.getItem('canonical-lifecycle');
if(persisted) canonical={...canonical,...JSON.parse(persisted)};
...
window.fetch = async (url, init) => {
  ...
  if (body.operation === 'list_summaries') return ok({v:1,workspaceId,summaries:[summary()],nextCursor:null});
  if (body.operation === 'csrf') return ok({ csrfToken: 'fixture-csrf', expiresAt: Math.floor(Date.now() / 1000) + 600 });
  if (body.operation === 'lookup') return body.mailboxId === 'main' && body.sourceRef.providerMessageId === 'source-123' ? ok({collaborationId:canonical.collaborationId}) : new Response('{}',{status:404});
  if (body.operation === 'read') return body.collaborationId === canonical.collaborationId ? ok({collaboration:canonical}) : new Response('{}',{status:404});
  ...
  else if (body.operation === 'resolve' || body.operation === 'reopen') { canonical = { ...canonical, updatedAt: canonical.updatedAt + 1, state: body.operation === 'resolve' ? 'resolved' : 'note_only' }; sessionStorage.setItem('canonical-lifecycle',JSON.stringify({state:canonical.state,updatedAt:canonical.updatedAt})); return ok({changed: true, collaboration: canonical}); }
...
function Harness({collaborationSummaries}) {
  ...
  const activeCollaborationOwnerProjection = collaborationOwnerProjection.collaboration ?? null;
  ...
  const selectCollaborationContextTab = tab => { setCollaborationContextTab(tab); if (tab === 'email') setHasCollaborationEmailOpened(true); };
  ...
  const exactNotificationProjectionRef = useRef(null), collaborationMessageRefs = useRef({});
  ...
  const [activeCollaborationMessageId,setActiveCollaborationMessageId]=useState(null);
  const [activeCollaborationSourceMailboxId,setActiveCollaborationSourceMailboxId]=useState(null);
  const activeCollaborationMessage=sourceMail;
  const open=activeCollaborationMessageId!==null;
  const activeCollaborationOwnerContextKey = collaborationOwnerProjection.identityKey, currentViewerPersistenceKey = 'fixture-viewer', currentMemberUserId = me, currentUserEmail = 'rutger@example.test';
  ...
  const onCanonicalCollaborationMutation = collaborationSummaries.acceptMutation;
  /* APPLY */
  const setCollaborationPersonIds=()=>{},setCollaborationMemberSearch=()=>{},setStartCollaborationInviteEmail=()=>{},setCollaborationNote=()=>{},setCollaborationParticipantPersonIds=()=>{},setCollaborationParticipantSearch=()=>{},setExternalCollaborationEmail=()=>{},setExternalCollaborationInviteUrl=()=>{},setExternalInviteEmailFeedback=()=>{},setExternalReviewCopyFeedback=()=>{},setCollaborationHistoryExpanded=()=>{},setCollaborationReplyDraft=()=>{},setCollaborationReplyFeedback=()=>{},setCollaborationReplySendingMessageId=()=>{},setCollaborationReplyVisibility=()=>{},setHasManualCollaborationReplyVisibility=()=>{},setCollaborationReplySelection=()=>{},setFocusCollaborationComposer=()=>{};
  const getMessageById=()=>sourceMail,clearCollaborationDraft=()=>{},hasRealInternalTeamContext=true;
  const collaborationOverlayOpenerRef=useRef(null);
  /* CLOSE_MODAL */
  useEffect(() => { if (isCollaborationSecureLinkVisible) setCollaborationContextTab('settings'); }, [isCollaborationSecureLinkVisible]);
  /* CLOSE */
  ...
  const mailbox = {id:'main',email:'rutger@example.test'};
  const workspaceDataMode='live',hasAuthenticatedMemberAuthority=true;
  const managedInboxes=[{id:'main',provider:'google',connected:true,connectionStatus:'connected'}];
  const collaborationSummaryScopeKey=collaborationSummaries.scopeKey;
  const authenticatedUser={workspaceId};
  const collaborationManagedMailboxById=new Map(managedInboxes.map(m=>[m.id,m]));
  const input=message=>({workspaceDataMode,hasAuthenticatedMemberAuthority,managedMailbox:managedInboxes[0],sourceMailboxId:'main',trustedFolder:'INBOX',message});
  /* HISTORY_GETTER */
  /* ACTIVE_GETTER */
  const unrelatedMail={...sourceMail,id:'unrelated',providerMessageId:'other-source'};
  const [selection,setSelection]=useState('source');
  const selectedMessage=selection==='source'?sourceMail:unrelatedMail;
  const currentMessageLocationByMessage=new Map([[sourceMail,{mailboxId:'main',folder:'Inbox'}],[unrelatedMail,{mailboxId:'main',folder:'Inbox'}]]);
  const buildCurrentMessageSelection=message=>({key:'main:'+message.providerMessageId});
  const isFullMessageOpen=false;
  /* BINDING */
  const currentCollaborationOpenBindingRef=useRef(null);
  currentCollaborationOpenBindingRef.current=getCollaborationOpenBinding(selectedMessage);
  const setCollaborationOwnerCreateState=()=>{},setPendingEndCollaborationMessageId=()=>{},setIsCollaborationParticipantPickerOpen=()=>{},setIsCollaborationInviteComposerOpen=()=>{},setIsCollaborationActionsMenuOpen=()=>{};
  /* FENCE */
  /* READ */
  const resolveCollaborationStorageMailboxId=(_,id)=>id??'main',getCollaborationLocalStateKey=id=>id;
  const syncMessageFromLiveSnapshot=()=>{},markCollaborationSeen=()=>{};
  const resolveAuthoritativeMessageLocation=({exactLocation})=>exactLocation;
  const currentMessageLocationById={mail:{mailboxId:'main',folder:'Inbox'}},draftCollaborationByMessageId={};
  const activeFolder='Inbox',setSelectionState=()=>setSelection('source');
  const setContextMenuState=()=>{},setDetailActionsMenuState=()=>{};
  /* OPEN */
  /* CTA */
  fixture.prioritySource=manualOverride=>composeCollaborationPrioritySource(resolvePrioritySource({message:sourceMail,manualOverride}),Boolean(getActiveCollaborationSummary('main',sourceMail,'Inbox')));
  fixture.indexSizes=()=>({active:collaborationSummaries.index.size,history:collaborationSummaries.historyIndex?.size});
  fixture.activePriority=()=>Boolean(getActiveCollaborationSummary('main',sourceMail,'Inbox'));
  fixture.state=()=>({canonical:canonical.state,history:getCollaborationSummary('main',sourceMail,'Inbox')?.state??null,active:getActiveCollaborationSummary('main',sourceMail,'Inbox')?.state??null,modal:activeCollaborationOwnerProjection?.state??null});
  fixture.canonicalId=()=>activeCollaborationOwnerProjection?.collaborationId;
  fixture.wrongSource=()=>getCollaborationSummary('main',{...sourceMail,providerMessageId:'wrong'},'Inbox');
  ...
  fixture.target = id => {
    fixture.marks = [];
    setCollaborationContextTab('conversation'); setHighlightedCollaborationMessageId(id);
    setServerNotificationDisplay({ collaboration: canonical, target: { message: { id: 'mail' } }, ticket: { isCurrent: () => true, notification: { activityId: id, mailboxId: 'main', collaborationId: canonical.collaborationId, sourceRef: {} } }, complete: displayed => fixture.marks.push({ displayed, focused: document.activeElement?.getAttribute('data-collaboration-activity-id') }) });
  };
  /* DISPLAY */
  if (!open) return <main data-theme={themeMode} className="min-h-screen bg-[var(--workspace-bg)] p-12 text-[var(--workspace-text)]"><h1 className="mb-3 text-xl">{sourceMail.subject}</h1><p className="mb-4">From {sourceMail.sender}</p><nav className="mb-4 flex gap-4"><button onClick={()=>setSelection('source')}>Select source mail</button><button onClick={()=>setSelection('unrelated')}>Select unrelated mail</button></nav><section data-selected-mail={selectedMessage.id}>{renderMessageCollaboration(selectedMessage)}</section></main>;
...
function WorkspaceProjection(){const collaborationSummaries=useCollaborationSummaries(workspaceId,me);return <Harness collaborationSummaries={collaborationSummaries}/>;}
createRoot(document.getElementById('root')).render(<WorkspaceProjection />);
