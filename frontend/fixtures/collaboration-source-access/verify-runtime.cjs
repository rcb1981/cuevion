...
(async()=>{
 const pre=process.argv.includes('--pre-fix');
 ...
 try{
  ...
  await shot(pre?'before-active':'active-source');
  ...
  if(pre){
   await check('PRE-FIX BUG: resolved source loses all Collaboration CTAs',async()=>{assert.equal(await page.locator('[data-open-collaboration]').count(),0);assert.equal((await state()).history,null);assert.equal((await state()).canonical,'resolved');});
   await shot('before-resolved-missing-cta');
   assert.deepEqual(errors,[]);assert.deepEqual(external,[]);
   fs.writeFileSync(path.join(__dirname,'pre-fix-results.json'),JSON.stringify({baseline:'0982786e7781d4f00e0d6113b29346c2e679883e',bugReproduced:true,checks,resolvedBeforeClose,afterClose:await state(),requests:await page.evaluate(()=>fixture.requests.map(({headers,...r})=>r)),errors,external},null,2)+'\n');
   return;
  }
