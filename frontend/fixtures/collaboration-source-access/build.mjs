...
const baseline=process.argv.includes('--baseline')?'0982786e7781d4f00e0d6113b29346c2e679883e':null;
const readBaseline=relative=>execFileSync('git',['show',baseline+':'+relative],{cwd:path.dirname(frontend),encoding:'utf8',maxBuffer:10*1024*1024});
const shell = baseline ? readBaseline('frontend/src/components/workspace/WorkspaceShell.tsx') : fs.readFileSync(shellPath, 'utf8');
...
function declaration(name) {
 const ast=ts.createSourceFile(shellPath,shell,ts.ScriptTarget.Latest,true,ts.ScriptKind.TSX);let result;
 function visit(node){if(ts.isVariableDeclaration(node)&&node.name.getText(ast)===name)result='const '+node.getText(ast)+';';ts.forEachChild(node,visit);}
 visit(ast);if(!result)throw Error('Missing '+name);return result;
}
const chunks = {
  HISTORY_GETTER: declaration('getCollaborationSummary'),
  ACTIVE_GETTER: declaration('getActiveCollaborationSummary'),
  CLOSE_MODAL: declaration('closeCollaborationOverlay'),
  FENCE: declaration('fenceCollaborationOwnerProjection'),
  APPLY: declaration('applyCanonicalCollaborationAccessResult'),
  CTA: declaration('renderMessageCollaboration'),
  BINDING: declaration('getCollaborationOpenBinding'),
  OPEN: declaration('openCollaborationOverlay'),
  READ: declaration('beginCollaborationOwnerRead'),
  MODAL: modal,
...
const out = '/private/tmp/cuevion-c3p2c1';
fs.mkdirSync(out, { recursive: true });
const readerExports = ['isCollaborationOwnerReadFailureRetryable','resolveMessageBodyRenderMode','EmailHtmlStage','renderPlainMessageParagraph','getQuotedParagraphStartIndex','parseQuotedSections','hasReliableQuotedContent','shouldShowInAttachmentList','shouldRenderKnownProviderHtmlOnLightCanvas','isComposeGeneratedHtml','resolveDesktopThreadTimestamp','normalizeSenderLearningKey','SignatureBlock'];
await build({ plugins: [{name: 'existing-reader', setup(api) {
  if(baseline) api.onLoad({filter:/useCollaborationSummaries\.ts$/},({path:loadedPath})=>{
    if(loadedPath!==path.join(frontend,'src/lib/useCollaborationSummaries.ts'))throw Error('Unexpected hook path');
    return {contents:readBaseline('frontend/src/lib/useCollaborationSummaries.ts'),loader:'ts'};
  });
...
}}], stdin: { contents: template, resolveDir: here, sourcefile: 'fixture.tsx', loader: 'tsx' }, bundle: true, format: 'esm', outfile: path.join(out, 'fixture.js'), jsx: 'automatic', define: { 'process.env.NODE_ENV': '"development"' } });
