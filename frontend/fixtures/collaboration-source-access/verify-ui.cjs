// Run after build.mjs with the fixture served on 127.0.0.1:4178.
const assert = require('node:assert/strict');
const fs = require('node:fs');
const path = require('node:path');
const screenshots='/private/tmp/cuevion-c3p2c1-ui';fs.mkdirSync(screenshots,{recursive:true});
...
(async () => {
  ...
  await p.route('**/*', route => { const url = route.request().url(); network.push(url); return url.startsWith('http://127.0.0.1:4178/') ? route.continue() : route.abort(); });
  const check = async (name, fn) => { await fn(); results.push(name); console.log('PASS', name); };
  const fresh = async (suffix = '') => { if(p.url().startsWith('http://127.0.0.1:4178'))await p.evaluate(()=>sessionStorage.clear()); await p.goto('http://127.0.0.1:4178/' + suffix); await p.getByRole('button',{name:'Open Collaboration',exact:true}).click(); await p.getByRole('tab', { name: 'Conversation',exact:true }).waitFor(); };
  ...
  try {
    ...
    for (const [w,h,name] of [[1600,1000,'desktop'],[1280,800,'laptop'],[768,1024,'tablet'],[375,812,'mobile']]) {
      ...
      await check(`${w}×${h}: no overflow, one content scroll, reachable composer, aligned bubbles, People`, async () => {
        ...
        await p.getByRole('tab',{name:'Settings',exact:true}).click();
        assert.ok(await p.getByRole('heading',{name:'Cuevion Team'}).isVisible());
        await p.getByRole('tab',{name:'Conversation',exact:true}).click();
        assert.equal(await p.getByRole('tab',{name:'Conversation',exact:true}).evaluate(n=>document.activeElement===n),true);
      });
      await p.screenshot({ path:path.join(screenshots,`conversation-${name}.png`), animations:'disabled' });
    }
    await p.setViewportSize({width:1600,height:1000});

    await check('exactly three semantic tabs; Conversation default; old source action absent',async()=>{
      assert.equal(await p.getByRole('tab').count(),3);assert.equal(await p.getByRole('tab',{name:'Conversation',exact:true}).getAttribute('aria-selected'),'true');
      assert.equal(await p.getByRole('button',{name:/People|Collaboration actions/}).count(),0);
      assert.equal(await p.getByRole('menu').count(),0);
    ...
    await check('tabs retain Shared/Internal drafts, composer mode, scroll and DOM nodes',async()=>{
      ...
      await p.getByRole('tab',{name:'Settings',exact:true}).click();await p.getByRole('tab',{name:'Conversation',exact:true}).click();assert.equal(await draft().inputValue(),'Preserved Internal');
    ...
    await check('keyboard arrows, Home and End select tabs and move focus',async()=>{
      ...
      await p.keyboard.press('Home');assert.equal(await conversation.getAttribute('aria-selected'),'true');await p.keyboard.press('End');assert.equal(await p.getByRole('tab',{name:'Settings',exact:true}).evaluate(n=>n===document.activeElement),true);await p.keyboard.press('ArrowRight');assert.equal(await conversation.getAttribute('aria-selected'),'true');await p.keyboard.press('ArrowLeft');assert.equal(await p.getByRole('tab',{name:'Settings',exact:true}).getAttribute('aria-selected'),'true');await p.getByRole('tab',{name:'Email',exact:true}).click();
    });
    ...
    for (const [w,h,name] of [[1600,1000,'desktop'],[1280,800,'laptop'],[768,1024,'tablet'],[375,812,'mobile']]) {
      ...
      await p.screenshot({path:path.join(screenshots,`email-${name}.png`),animations:'disabled'});
    }
    ...
    await check('Resolve requires confirmation, Cancel default, focus trapped, Cancel and Escape do not resolve',async()=>{
      const open=async()=>{await p.getByRole('tab',{name:'Settings',exact:true}).click();await p.getByRole('button',{name:'Resolve collaboration',exact:true}).click();await p.getByRole('dialog',{name:'Resolve this collaboration?'}).waitFor();};
      await open();assert.equal(await p.getByRole('button',{name:'Cancel',exact:true}).evaluate(n=>n===document.activeElement),true);
      await p.screenshot({path:path.join(screenshots,'resolve-confirmation.png'),animations:'disabled'});
      ...
      await p.getByRole('button',{name:'Cancel',exact:true}).click();assert.equal(await p.getByRole('button',{name:'Resolve collaboration',exact:true}).evaluate(n=>n===document.activeElement),true);
    ...
    for (const [w,h,label] of [[1600,1000,'desktop'],[375,812,'mobile']]) {
      ...
      await check(`dark ${label}: Conversation, Email, Settings/dialog and focus visible`,async()=>{
        ...
        await p.screenshot({path:path.join(screenshots,`dark-conversation-${label}.png`),animations:'disabled'});
        await p.getByRole('tab',{name:'Email',exact:true}).click();await p.frameLocator('iframe[title="Email content"]').getByRole('heading',{name:'Ready for the final listen'}).waitFor();await p.locator('#collaboration-panel-email').evaluate(n=>{n.scrollTop=0;});await p.screenshot({path:path.join(screenshots,`dark-email-${label}.png`),animations:'disabled'});
        await p.getByRole('tab',{name:'Settings',exact:true}).click();await p.screenshot({path:path.join(screenshots,`dark-settings-${label}.png`),animations:'disabled'});
        await p.getByRole('button',{name:'Resolve collaboration',exact:true}).click();await p.screenshot({path:path.join(screenshots,`dark-confirmation-${label}.png`),animations:'disabled'});await p.getByRole('button',{name:'Cancel',exact:true}).click();
      });
    }
    await fresh();await p.setViewportSize({width:1600,height:1000});
    ...
    for (const [id,name] of [['B','Shared'],['E','Internal'],['D','Guest']]) await check(`${name} C3D exact ref, focus, highlight before displayed acknowledgement`,async()=>{
      await p.getByRole('tab',{name:'Settings',exact:true}).click();await p.evaluate(id=>fixture.target(id.repeat(22)),id);await waitFor(()=>fixture.marks.length>0);
    ...
    await check('exact source: active CTA, Resolve once, close, resolved CTA, exact lookup/read, Reopen and Priority',async()=>{
      await p.getByRole('button',{name:'Close collaboration',exact:true}).click();await p.getByRole('button',{name:'Open Collaboration',exact:true}).click();
      await p.locator('[data-collaboration-owner-read-projection]').waitFor();
      assert.equal(await p.evaluate(()=>fixture.activePriority()),true);
      assert.equal(await p.evaluate(()=>fixture.wrongSource()),null);
      await p.getByRole('tab',{name:'Settings',exact:true}).click();
      await p.screenshot({path:path.join(screenshots,'settings-active-desktop.png'),animations:'disabled'});
      await p.getByRole('button',{name:'Resolve collaboration',exact:true}).click();await p.getByRole('button',{name:'Resolve',exact:true}).evaluate(n=>{n.click();n.click();});
      await p.getByRole('button',{name:'Reopen collaboration',exact:true}).waitFor();assert.equal(await p.locator('[data-collaboration-lifecycle-status]').innerText(),'Resolved');
      assert.equal(await p.evaluate(()=>fixture.activePriority()),false);
      await p.screenshot({path:path.join(screenshots,'settings-resolved-desktop.png'),animations:'disabled'});
      await p.getByRole('button',{name:'Close collaboration',exact:true}).click();await p.getByRole('button',{name:'View Collaboration',exact:true}).waitFor();
      await p.screenshot({path:path.join(screenshots,'resolved-mail-cta.png'),animations:'disabled'});
      const count=await p.evaluate(()=>fixture.requests.length);
      await p.getByRole('button',{name:'View Collaboration',exact:true}).focus();await p.keyboard.press('Enter');await p.locator('[data-collaboration-owner-read-projection]').waitFor();
      assert.equal(await p.getByRole('tab',{name:'Conversation',exact:true}).getAttribute('aria-selected'),'true');assert.equal(await p.evaluate(()=>fixture.canonicalId()),'C'.repeat(22));
      const read=await p.evaluate(n=>fixture.requests.slice(n).filter(r=>r.operation!=='csrf'),count);
      assert.deepEqual(read.map(r=>r.operation),['lookup','read']);assert.deepEqual(read[0].sourceRef,{providerMessageId:'source-123'});assert.equal(read[1].collaborationId,'C'.repeat(22));
      await p.getByRole('tab',{name:'Email',exact:true}).click();await p.frameLocator('iframe').getByRole('heading',{name:'Ready for the final listen'}).waitFor();
      await p.getByRole('tab',{name:'Settings',exact:true}).click();assert.equal(await p.locator('[data-collaboration-lifecycle-status]').innerText(),'Resolved');
      await p.evaluate(()=>fixture.target('E'.repeat(22)));await waitFor(()=>fixture.marks.length>0);assert.ok((await p.evaluate(()=>fixture.marks)).every(m=>m.displayed&&m.focused==='E'.repeat(22)));
      assert.equal(await p.getByRole('tab',{name:'Conversation',exact:true}).getAttribute('aria-selected'),'true');
      await p.getByRole('tab',{name:'Settings',exact:true}).click();await p.getByRole('button',{name:'Reopen collaboration',exact:true}).click();await p.getByRole('button',{name:'Resolve collaboration',exact:true}).waitFor();
      assert.equal(await p.evaluate(()=>fixture.activePriority()),true);assert.equal(await p.locator('[data-collaboration-lifecycle-status]').innerText(),'Active');
      ...
      await p.getByRole('button',{name:'Close collaboration',exact:true}).click();await p.getByRole('button',{name:'Open Collaboration',exact:true}).click();await p.locator('[data-collaboration-owner-read-projection]').waitFor();
    });
    for (const [w,h,name] of [[1600,1000,'desktop'],[1280,800,'laptop'],[768,1024,'tablet'],[375,812,'mobile']]) {
      await p.setViewportSize({width:w,height:h});await p.getByRole('tab',{name:'Settings',exact:true}).click();
      await check(`Settings ${w}×${h}: controls reachable, tabs fit, single outer scroller`,async()=>{
        assert.ok(await p.getByRole('button',{name:'Invite external guest',exact:true}).isVisible());
        await p.getByRole('button',{name:'Resolve collaboration',exact:true}).scrollIntoViewIfNeeded();
        const layout=await p.evaluate(()=>({width:document.documentElement.scrollWidth,height:document.documentElement.scrollHeight,scrolls:[...document.querySelector('[data-collaboration-thread-modal]').querySelectorAll('*')].filter(n=>n.offsetParent&&n.scrollHeight>n.clientHeight+1&&/auto|scroll/.test(getComputedStyle(n).overflowY)).length}));
        assert.ok(layout.width<=w&&layout.height<=h&&layout.scrolls<=1);
      });
      await p.locator('#collaboration-panel-settings').evaluate(n=>{n.scrollTop=0;});
      if(name==='mobile')await p.screenshot({path:path.join(screenshots,'settings-mobile.png'),animations:'disabled'});
    }
    await p.setViewportSize({width:1600,height:1000});
    await check('People exposes historical guests and adds an eligible Team member',async()=>{
      await p.getByRole('tab',{name:'Settings',exact:true}).click();assert.ok(await p.getByText('Previous reviewer',{exact:true}).isVisible());assert.ok(await p.getByText('Revoked',{exact:true}).isVisible());
      await p.getByRole('button',{name:'Add Team member',exact:true}).click();await p.getByRole('combobox',{name:'Team member'}).selectOption({label:'Dan — dan@example.test'});await p.getByRole('button',{name:'Add Team member',exact:true}).last().click();await p.getByText('Dan',{exact:true}).waitFor();
    });
    ...
    await check('People toggling preserves link and overlay close protection',async()=>{
      const link=await p.locator('[data-collaboration-secure-link] input').inputValue();await p.getByRole('tab',{name:'Conversation',exact:true}).click();
      ...
      await p.getByRole('tab',{name:'Settings',exact:true}).click();assert.equal(await p.locator('[data-collaboration-secure-link] input').inputValue(),link);
      assert.deepEqual(await p.evaluate(()=>[localStorage.length,Object.keys(sessionStorage).filter(k=>k!=='canonical-lifecycle').length]),[0,0]);
    });
    ...
    await check('no browser errors or external requests',async()=>{assert.deepEqual(errors,[]);assert.ok(network.every(u=>u.startsWith('http://127.0.0.1:4178/')));});
    fs.writeFileSync(path.join(__dirname,'ui-results.json'),JSON.stringify({passed:results.length,tests:results,errors,network:[...new Set(network)]},null,2)+'\n');
