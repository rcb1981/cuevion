// Run after build.mjs with the fixture served on 127.0.0.1:4177.
...
(async () => {
  ...
  await p.route('**/*', route => { const url = route.request().url(); network.push(url); return url.startsWith('http://127.0.0.1:4177/') ? route.continue() : route.abort(); });
  const check = async (name, fn) => { await fn(); results.push(name); console.log('PASS', name); };
  const fresh = async (suffix = '') => { await p.goto('http://127.0.0.1:4177/' + suffix); await p.getByRole('tab', { name: 'Conversation',exact:true }).waitFor(); };
  ...
  try {
    ...
    for (const [w,h,label] of [[1600,1000,'desktop'],[375,812,'mobile']]) {
      ...
      await check(`dark ${label}: Conversation, Email, Settings/dialog and focus visible`,async()=>{
        ...
        await p.getByRole('tab',{name:'Settings',exact:true}).click();await p.screenshot({path:path.join(__dirname,`dark-settings-${label}.png`),animations:'disabled'});
        await p.getByRole('button',{name:'Resolve collaboration',exact:true}).click();await p.screenshot({path:path.join(__dirname,`dark-confirmation-${label}.png`),animations:'disabled'});await p.getByRole('button',{name:'Cancel',exact:true}).click();
      });
    }
    await fresh();await p.setViewportSize({width:1600,height:1000});
    ...
    await check('exact source: active CTA, Resolve once, close, resolved CTA, exact lookup/read, Reopen and Priority',async()=>{
      ...
      await p.getByRole('tab',{name:'Settings',exact:true}).click();
      await p.screenshot({path:path.join(__dirname,'settings-active-desktop.png'),animations:'disabled'});
      ...
      await p.screenshot({path:path.join(__dirname,'settings-resolved-desktop.png'),animations:'disabled'});
      ...
      await p.screenshot({path:path.join(__dirname,'resolved-mail-cta.png'),animations:'disabled'});
    ...
    for (const [w,h,name] of [[1600,1000,'desktop'],[1280,800,'laptop'],[768,1024,'tablet'],[375,812,'mobile']]) {
      ...
      if(name==='mobile')await p.screenshot({path:path.join(__dirname,'settings-mobile.png'),animations:'disabled'});
    }
    await p.setViewportSize({width:1600,height:1000});
    ...
    await check('no browser errors or external requests',async()=>{assert.deepEqual(errors,[]);assert.ok(network.every(u=>u.startsWith('http://127.0.0.1:4177/')));});
