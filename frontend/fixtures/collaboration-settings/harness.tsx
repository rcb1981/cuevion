...
const summaryStore=createCollaborationSummaryStore(workspaceId,async()=>({status:'success',page:{v:1,workspaceId,summaries:[summary()],nextCursor:null}}));
...
function Harness() {
  ...
  useEffect(()=>summaryStore.subscribe(()=>setSummaryVersion(v=>v+1)),[]);
  const onCanonicalCollaborationMutation = collaboration => { void summaryStore.acceptMutation(collaboration); };
  ...
  const collaborationSummaryScopeKey='fixture-workspace';
  ...
  const getCollaborationSummary=(mailboxId,message)=>lookupCollaborationSummary(summaryStore.getSnapshot(),workspaceId,input(message));
  const currentMessageLocationByMessage=new Map([[sourceMail,{mailboxId:'main',folder:'Inbox'}]]);
  ...
  currentCollaborationOpenBindingRef.current=getCollaborationOpenBinding(sourceMail);
  ...
  /* READ */
  const openCollaborationOverlay=(id,options)=>{setCollaborationContextTab('conversation');setHasCollaborationEmailOpened(false);setOpen(true);beginCollaborationOwnerRead(id,options.sourceMailboxId,options.sourceMessage,'Inbox',options.expectedBinding);};
  /* CTA */
  fixture.activePriority=()=>Boolean(lookupActiveCollaborationSummary(summaryStore.getSnapshot(),workspaceId,input(sourceMail)));
  fixture.canonicalId=()=>activeCollaborationOwnerProjection?.collaborationId;
  fixture.wrongSource=()=>lookupCollaborationSummary(summaryStore.getSnapshot(),workspaceId,input({...sourceMail,providerMessageId:'wrong'}));
  ...
  /* DISPLAY */
  if (!open) return <main data-theme={themeMode} className="min-h-screen bg-[var(--workspace-bg)] p-12 text-[var(--workspace-text)]"><h1 className="mb-3 text-xl">{sourceMail.subject}</h1><p className="mb-4">From {sourceMail.sender}</p>{renderMessageCollaboration(sourceMail)}</main>;
