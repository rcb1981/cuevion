// Run after build.mjs with the fixture served on 127.0.0.1:4179.
const assert = require('node:assert/strict');
const fs = require('node:fs');
const path = require('node:path');
const screenshots='/private/tmp/cuevion-c3p2d-ui';fs.mkdirSync(screenshots,{recursive:true});
...
(async () => {
  ...
  await p.route('**/*', route => { const url = route.request().url(); network.push(url); return url.startsWith('http://127.0.0.1:4179/') ? route.continue() : route.abort(); });
  const check = async (name, fn) => { await fn(); results.push(name); console.log('PASS', name); };
  const fresh = async (suffix = '') => { if(p.url().startsWith('http://127.0.0.1:4179'))await p.evaluate(()=>sessionStorage.clear()); await p.goto('http://127.0.0.1:4179/' + suffix); await p.getByRole('button',{name:'Open Collaboration',exact:true}).click(); await p.getByRole('tab', { name: 'Conversation',exact:true }).waitFor(); };
  ...
  try {
    ...
    await check('no browser errors or external requests',async()=>{assert.deepEqual(errors,[]);assert.ok(network.every(u=>u.startsWith('http://127.0.0.1:4179/')));});
