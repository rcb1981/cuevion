...
r"""CREATE TABLE cuevion_mailbox.mailbox_sync_cursor (
    ...
    CONSTRAINT ck_mailbox_sync_cursor_scope_key_digest_canonical CHECK (scope_key_digest ~ '^[0-9a-f]{64} CHECK (backfill_state IN ('not_started','running','complete','recovering','blocked')),
...
r"""CREATE TABLE cuevion_mailbox.mailbox_messages (
    ...
    CONSTRAINT ck_mailbox_messages_provider_folder_digest_canonical CHECK (provider_folder_digest ~ '^[0-9a-f]{64} CHECK ((provider = 'google' AND provider_message_id IS NOT NULL AND imap_uid_validity IS NULL AND imap_uid IS NULL) OR (provider = 'custom_imap' AND provider_message_id IS NULL AND imap_uid_validity IS NOT NULL AND imap_uid IS NOT NULL)),
